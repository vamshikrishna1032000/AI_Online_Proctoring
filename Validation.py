...
ResultSheetRows=ResultSheet.rows
ResultSheetHeaders=[cell.value for cell in next(ResultSheetRows)]

face_model = get_face_detector()
landmark_model = get_landmark_model()

for row in ResultSheetRows:
    print(row[0].value)
    EyetrackerList=[]
    MouthtrackerList=[]
    HeadtrackerList=[]
    cap = cv2.VideoCapture("./dataset/"+str(row[0].value)+".mp4")
    ret, img = cap.read()
    size = img.shape
    ...
    while cap.isOpened():
        ...
        for rect in rects:
            ...
            cv2.putText(img, 'Press r to record distances', (30, 30), font,
                        1, (0, 255, 255), 2)
            cv2.imshow("Output", img)
        if cv2.waitKey(1) & i<1:
            for i in range(100):
            ...
            if(faces.__len__()>0):
                ...
                for rect in rects:
                    ...
                    if cnt_outer > 3 and cnt_inner > 2:
                        ...
                        cv2.putText(img, 'Mouth open', (30, 70), font,
                                1, (0, 255, 255), 2)
                    else:
    ...
    NoOfFrames=min(len(EyetrackerList),len(HeadtrackerList),len(MouthtrackerList))
    SecondWIseValidationList=[]
    NoOfSeconds=0
    for i in range(0,NoOfFrames+1,fps):
        FrameValidationList=[]
        maxlim=min(i+fps,NoOfFrames)
        NoOfSeconds=NoOfSeconds+1
        for j in range(i,maxlim):
            if(EyetrackerList[j]!='Legal'or HeadtrackerList[j]!='Legal' or HeadtrackerList[j]!='Legal'):
    ...
    if(SecondWIseValidationList.count('Illegal')>7):
        row[2].value='cheating' 
    else:
        row[2].value='not cheating'     
    row[3].value= SecondWIseValidationList.count('Illegal')
    row[4].value= NoOfSeconds
    ResultBook.save('result.xlsx')
    cv2.destroyAllWindows()
    cap.release()
